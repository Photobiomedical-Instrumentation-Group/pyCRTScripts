...
def loadCachedVideo(videoPath):
    ...
    with np.load(cachePath) as data:
        ...
        avgGArr = data["avgGArr"]

    VIDEO_CACHE[videoPath] = (cachedVideo, timesScdsArr, avgGArr)
...
def selectFrames(videoPath, params, filterType=FILTER_TYPE, show=False):
    ...
    cachedVideo, timesScdsArr, avgGArr = loadCachedVideo(videoPath)
...
def plotVisualization(
    ...
    # {{{
    cachedVideo, timesScdsArr, avgGArr = VIDEO_CACHE[videoPath]
    ...
    ax1.plot(timesScdsArr, avgGArr, lw=1, color="tab:green", label="avg G")
    ...
    try:
        avgGDiagnostics = calcSignalGradientDiagnostics(
            avgGArr,
            timesScdsArr,
        ...
        print(f"{videoPath.stem}: G-gradient diagnostics failed: {err}")
        avgGDiagnostics = None
    else:
        ax1.axvspan(
            ...
            color="tab:gray",
            alpha=0.12,
            label="G low-gradient interval",
        )
        avgGRange = avgGArr.max() - avgGArr.min()
        avgGIntervalSmoothRawScale = (
            avgGDiagnostics["signalInterval"] * avgGRange + avgGArr.min()
        )
        ax1.plot(
            avgGDiagnostics["times"],
            avgGIntervalSmoothRawScale,
            color="tab:orange",
            lw=1.4,
            label="smoothed G slice",
        )
        ax1Grad = ax1.twinx()
        ax1Grad.plot(
            avgGDiagnostics["fullTimes"],
            avgGDiagnostics["fullSignalGrad"],
            color="tab:gray",
            lw=0.8,
            alpha=0.75,
            label="full grad norm G",
        )
        ax1Grad.axhline(
            avgGDiagnostics["relaxMinGrad"],
            color="tab:red",
            lw=0.8,
            ls=":",
            label=f"relaxMinGrad={avgGDiagnostics['relaxMinGrad']:.3f}",
        )
        ax1Grad.axhline(
            avgGDiagnostics["relaxMaxGrad"],
            color="tab:red",
            lw=0.8,
            ls=":",
            label=f"relaxMaxGrad={avgGDiagnostics['relaxMaxGrad']:.3f}",
        )
        ax1Grad.axhline(
            avgGDiagnostics["strictMinGrad"],
            color="tab:orange",
            lw=0.8,
            ls=":",
            label=f"strictMinGrad={avgGDiagnostics['strictMinGrad']:.3f}",
        )
        ax1Grad.axhline(
            avgGDiagnostics["strictMaxGrad"],
            color="tab:orange",
            lw=0.8,
            ls=":",
            label=f"strictMaxGrad={avgGDiagnostics['strictMaxGrad']:.3f}",
        )
    ...
    crtArrRaw = avgGArr[crtIntervalStartLocalIndex:crtIntervalEndLocalIndex]
    ...
    try:
        ...
        axCRT90_10.plot(
            ...
            lw=2,
            label=(
                f"Gaussian-smoothed G (sigma={CRT90_10_GAUSSIAN_SIGMA_SECONDS:.3f}s)"
            ),
        )
        axCRT90_10.axvline(
            crt90_10["startTime"],
    ...
    pcrtFit = calcPCRTFit(timesScdsArr, avgGArr, localIndices)
    ...
    if avgGDiagnostics is not None:
        avgGGrad = avgGDiagnostics["signalGrad"]
        print(f"{videoPath.stem} {avgGGrad.min():.3f}, {avgGGrad.max():.3f}")
